 ...
 * Facturacion model.
 */
class FacturacionModelFacturacion extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 * @since	1.6
	 */
	protected $text_prefix = 'COM_FACTURACION';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_facturacion.facturacion', 'facturacion', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM facturacion');
				$max = $db->loadResult();
