function llenar_tabla_comprobantes(data) {
	...
	$j.each( data, function( i, val ) {
		...
		if(val.id_tipo_comp == "Venta"){
			codigo_tabla = codigo_tabla+"<a href='index.php?option=com_facturacion&task=facturacionform.bajaComprobante&id_comp="+val.id_comprobante+"' title='Dar de Baja'  target='_self'><i class='icon-remove'></i></a>";
		}
